...
class Bomb {
    ...
    update = () => {
        let upt = this.time / 11
        if (Number.isInteger(upt))
            if (upt < 15) {
                let n = upt % 3
                this.bmp.divide.x = n * 44
            }
        if (upt >= 15) this.boomUpdate()
        this.time++
    }

    boomUpdate = () => {//启动爆炸
        let bombt = this.boomtime / 2
        if (Number.isInteger(bombt)) {
            if (this.boomtime == 0) {
    ...
    changeEffect = (n) => {//改变水波贴图
        const frame = Math.min(parseInt(n / 2), 2)
        let m = n
        for (let i = 0; i < this.bd.up && i < 3 + (this.bd.up - 3) / 2 * frame; i++) {
            let r1 = i + 1
            this.bd.upEffects[i].cover = false
            this.bd.upEffects[i].pos = new Pos(this.y * 40 - 0.5, (this.x - r1) * 40 - 2)
            this.bd.upEffects[i].divide = new Pos(m * 40, 0 * 40)
        }
        for (let i = 0; i < this.bd.down && i < 3 + (this.bd.down - 3) / 2 * frame; i++) {
            let r1 = i + 1
            this.bd.downEffects[i].cover = false
            this.bd.downEffects[i].pos = new Pos(this.y * 40 - 0.5, (this.x + r1) * 40 - 2)
            this.bd.downEffects[i].divide = new Pos(m * 40, 1 * 40)
        }
        for (let i = 0; i < this.bd.left && i < 3 + (this.bd.left - 3) / 2 * frame; i++) {
            let r1 = i + 1
            this.bd.leftEffects[i].cover = false
            this.bd.leftEffects[i].pos = new Pos((this.y - r1) * 40 - 0.5, (this.x) * 40 - 2)
            this.bd.leftEffects[i].divide = new Pos(m * 40, 2 * 40)
        }
        for (let i = 0; i < this.bd.right && i < 3 + (this.bd.right - 3) / 2 * frame; i++) {
            let r1 = i + 1
            this.bd.rightEffects[i].cover = false
            this.bd.rightEffects[i].pos = new Pos((this.y + r1) * 40 - 0.5, (this.x) * 40 - 2)
            this.bd.rightEffects[i].divide = new Pos(m * 40, 3 * 40)
        }
        this.bd.centerEffects[0].pos = new Pos((this.y) * 40 - 0.5, (this.x) * 40 - 2)
        this.bd.centerEffects[0].divide = new Pos(m * 40, 4 * 40)
    }
